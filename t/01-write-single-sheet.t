...
#
#     - Call the command line script to run a query to select that
...
my $command = "perl $script_path --db='$temp_filename' --query='SELECT * FROM data' --excel='$excel_filename'";
system( $command );
####
...
is( $excel_data->[0]->{sheets}, 1, 'there is one sheet in the excel file' );

my $sheet = $excel_data->[1];

is_deeply( [ Spreadsheet::Read::rows( $sheet ) ], [ @data ], 'data in excel file matches data in database');
